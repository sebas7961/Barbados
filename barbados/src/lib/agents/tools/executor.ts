...
import { buildDateSelectorPayload } from '@/lib/whatsapp/provider'
...
export async function executeTool(name: string, args: Record<string, unknown>): Promise<unknown> {
    ...
    switch (name) {
        ...
        case 'mostrarSelectorFecha':
            const { telefono } = args as { telefono: string }
            await buildDateSelectorPayload(telefono)
            return { success: true, mensaje: 'Selector de fecha enviado al cliente' }
        default:
